...
- **推奨サイズ**: 高品質な画像を作成するため、6倍サイズ（1440×1920px）での作成を推奨
...
1. 新規ファイルを作成
2. サイズを **1440×1920px**（推奨）または **240×320px** に設定
...
**解決方法**:
1. 画像編集ツールでキャンバスサイズを **1440×1920px**（推奨）または **240×320px** のアスペクト比 3:4 に変更
2. パーツの位置を調整
3. 再度書き出してアップロード
...
2. **適切な解像度で作成**:
   - 最初から **1440×1920px**（推奨）または **240×320px** で作成
   - または高解像度で作成して縮小
3. **エッジを滑らかにする**:
   - 1-2px のぼかしを適用
   - フェザリングを使用
...
- [ ] **サイズ**: 1440×1920px（推奨）または 240×320px、アスペクト比 3:4
- [ ] **形式**: PNG（.png）
- [ ] **透過**: 背景以外はアルファチャンネルあり
- [ ] **エッジ**: アンチエイリアス処理済み

...
- **推奨サイズ**: 
  - キャラクターパーツ（顔・髪・衣装）: 6倍サイズ（1440×1920px）
  - 背景: 3倍サイズ（720×960px）
...
- キャンバス全体を覆うように配置
- 固定表示（キャラクターのような拡大縮小はされない）

**推奨サイズ**: 720×960px（3倍サイズ）
- PNG保存時の高画質出力（3倍解像度）に対応
...
> 💡 **推奨**: UI上の拡大機能（最大2倍）とPNG保存時の高画質出力（3倍解像度）を考慮し、6倍サイズ（1440×1920px）で作成することを推奨します。アップロード時に自動的に表示サイズ（240×320px）に調整されます。
...
> 💡 **推奨**: 背景は固定表示（拡大縮小なし）のため、PNG保存時の高画質出力（3倍解像度）のみを考慮し、3倍サイズ（720×960px）で作成することを推奨します。

> ⚠️ **重要**: アスペクト比 3:4 を維持してください。異なるアスペクト比で作成すると、パーツが正しく配置されない可能性があります。
...
1. 新規ファイルを作成
2. サイズを設定:
   - **キャラクターパーツ（顔・髪・衣装）**: **1440×1920px**（推奨）または **240×320px**
   - **背景**: **720×960px**（推奨）または **240×320px**
...
**解決方法**:
1. 画像編集ツールでキャンバスサイズをアスペクト比 3:4 に変更
   - キャラクターパーツ: **1440×1920px**（推奨）または **240×320px**
   - 背景: **720×960px**（推奨）または **240×320px**
2. パーツの位置を調整
3. 再度書き出してアップロード
...
**解決方法**:
1. **PNG 圧縮ツールを使用**:
   - PNG圧縮ツール（ローカル処理のものを推奨）を使用してファイルサイズを削減

...
**⚠️ 重要**: 開発環境では `sequelize.sync({ alter: true })` により、新しいカラムが自動的に追加されます。マイグレーションの手動実行は通常不要です。

#### 開発環境での自動マイグレーション

開発環境（`NODE_ENV` が `production` でない場合）では、サーバー起動時に以下の処理が自動実行されます：

- `sequelize.sync({ alter: true })` により、モデル定義に基づいて新しいカラムが自動追加
- 既存のデータは保持されたまま、スキーマのみが更新されます
- これにより、ブランチを切り替えた際も新しいカラムが自動的に追加されます

**注意**: 本番環境では `alter: true` は無効化されており、明示的なマイグレーション実行が必要です。

#### 既存データベースへのスキーマ変更適用（本番環境）

本番環境で新しいカラムやテーブルを追加する場合は、マイグレーションを明示的に実行します。

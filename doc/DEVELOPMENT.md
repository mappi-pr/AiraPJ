...
#### ステッカー
- `GET /api/sticker` - ステッカー一覧取得
- `POST /api/sticker/upload` - ステッカーアップロード
- `DELETE /api/sticker/:id` - ステッカー削除
...
#### photo（フォト撮影）画面
- 選択中のパーツ（背景・後髪・衣装・顔・前髪・ステッカー）を重ね合わせ表示
- レイヤー順: 背景 → 後髪 → 衣装 → 顔 → 前髪 → ステッカー
- スライダーでキャラクター（顔系+衣装）の拡大率を調整
- **ステッカー機能**:
  - 「ステッカー追加」ボタンでステッカー選択パネルを開く
  - 利用可能なステッカーから選択して画面に配置
  - 各ステッカーはドラッグ&ドロップで自由に移動可能
  - 各ステッカーは個別に拡大縮小可能（0.1倍～2倍）
  - 配置済みステッカーリストから削除可能
  - ステッカーは最前面レイヤー（z-index: 100+）に表示
- 「PNGで保存」ボタンで合成画像をダウンロード
  - `html2canvas` による画面キャプチャ方式（ステッカーも含む）
